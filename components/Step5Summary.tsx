...
const Step5Summary: React.FC<Props> = ({ data, onMiscChange, onAdvanceChange }) => {
  ...
  // Calculations
  // Fix: Property 'jarak' does not exist on type 'JourneyGroup'. Calculate total distance from journey legs (pergi and balik).
  const totalKm = data.logs.reduce((sum, l) => sum + l.pergi.jarak + (l.adaBalik ? l.balik.jarak : 0), 0);
