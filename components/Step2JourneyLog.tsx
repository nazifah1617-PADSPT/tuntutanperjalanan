...
const Step2JourneyLog: React.FC<Props> = ({ logs, onChange }) => {
  // Add a new journey record using the JourneyGroup structure
  const addJourney = () => {
  ...
  // Update a specific leg (pergi or balik) in a journey record
  const updateLeg = (id: string, legType: 'pergi' | 'balik', field: keyof JourneyLeg, value: any) => {
    onChange(logs.map(j => {
      if (j.id === id) {
        let updatedLeg = { ...j[legType], [field]: value };
        
        // Auto-calculate arrival time if departure time or duration inputs change
        ...
        // Sync destination of 'pergi' to starting point of 'balik' for convenience
